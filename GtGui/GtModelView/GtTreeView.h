...
#ifndef GT_TREE_VIEW_H
#define GT_TREE_VIEW_H
...
namespace GT
{
	namespace GtGui
	{
		...
		typedef HtlTreeNode<GtTreeModelIndex,void*>* CellNodeIter;
		typedef HtlTreeNode<Htl1DIndex,GtTableRow*>* CellRowIter;
		typedef HtlTreeNode<Htl1DIndex,GtTableColumn*>* CellColIter;

		class HTL_DLLAPI GtTreeView : public GtFrame
		{
		public:
			//!Parent set constructor
			GtTreeView(GtObject* ptrParent = NULL);
			//!Virtual destructor
			virtual ~GtTreeView(void);
			...
			//!The indent size between levels in pixels
			GTOBJECT_MEMVAR_BYVAL(public, int, intTab);
			//!The indent size between levels in pixels
			GTOBJECT_MEMVAR_BYVAL(public, bool, blnDrawGrid);
			//SIGNALS/////////////////////////////////
		public:
		...
		public:
			...
			void SetTreeModel(GtTreeItemModel * ptrModel);
			//Get the pointer to the Table model being viewed
			GtTreeItemModel * GetTreeModelPtr(void);
			//!Force a model reset
			void Reset(void);
			//!Get the pointer to the selection model
			GtTreeSelection* GetSelection(void);
			...
			bool IsNodeVisible(long row);
			//CELL MEMBER FUNCTIONS////////////////////////////
			//!Get the pointer to the cell at location
			GtTreeModelIndex* GetCellPtr(size_t index);
			//!Get the pointer to the cell at location
			GtTreeModelIndex* GetCellPtr(long row, long col);
			...
			GtTreeModelIndex* GetCellFromPt(GtPoint3DI pos);
		...
		public:
			...
			//!Paint an individual node
			void OnPaintNode(GtPainter * painter, GtTreeModelIndex* ptrNode, size_t & lngCurrRow );
		protected:
			...
			GtTreeSelection m_objSelection;
			...
			void UpdateTreeGeometry(void);
			//update the current Table cell locations
			void UpdateNodeGeometry(GtTreeModelIndex* ptrNode,size_t & lngCurrRow, GtRectI & rectLast);
