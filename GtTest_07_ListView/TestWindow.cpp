...
: GtMainWindow(ptrParent)
{
	...
	m_ptrSprocketModel = new SprocketListModel(this);
...
void TestWindow::InitializeControls(void)
{
	...
	if(!m_lstSprockets)
	{
		m_lstSprockets = new GtListView(this);
		rect.xMin = 200;
		rect.xMax = 600;
		rect.yMin = 50;
		rect.yMax = 500;
		if(m_lstSprockets)
		{
			m_lstSprockets->Set_objFrame(rect);
			m_lstSprockets->Set_objBackBrush(GtBrush(GtColor(190,90,160)));
			strCmd = "ListView";
			m_lstSprockets->Set_strText(strCmd);
			
			m_lstSprockets->SetListModel(m_ptrSprocketModel);
			this->AddSubWidget(m_lstSprockets);	
		}
	}
...
void TestWindow::InitializeActions(void)
{
	...
	if(m_lstSprockets)
	{
		m_lstSprockets->SelectionChanged.SlotConnect(this,&TestWindow::OnLoadSprocket);
	}
};
...
void TestWindow::OnLoadSprocket(void)
{
	...
	GtListModelIndex* ptrSel = m_lstSprockets->GetSelection()->Get_ptrTopLeft();
	if(!ptrSel){return;};
