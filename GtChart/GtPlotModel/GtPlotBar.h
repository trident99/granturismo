...
#ifndef GT_PLOT_BAR_H
#define GT_PLOT_BAR_H
...
namespace GT
{
	namespace GtChart
	{
		...
		class HTL_DLLAPI GtBarItem
		{
		public:
			GtBarItem();	
			~GtBarItem();
			//!Assignment Operator
			GtBarItem & operator = (const GtBarItem & rhs);
		...
		class HTL_DLLAPI  GtPlotBar: public GtPlotItem
		{
		public:
			//!Parent set constructor
			GtPlotBar(GtObject* ptrParent = NULL);
			//!Virtual destructor
			virtual ~GtPlotBar(void);
			//PUBLIC OPERATOR
			//!Assignment Operator
			GtPlotBar & operator = (GtPlotBar & rhs);
			...
			inline GtBarItem & At(size_t index);
			//!Adds the point3d the the back of the coordinates
			inline void PushBack(GtBarItem & objNewPoint);
		...
		public:
			...
			StlVector<GtBarItem, HtlUnknownType> m_arrObjPoints;
			std::string m_strObjectName;
		private:
			GtBarItem m_objNullPoint;
