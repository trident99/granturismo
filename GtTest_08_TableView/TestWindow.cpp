...
: GtMainWindow(ptrParent)
{
	...
	m_ptrSprocketModel = new SprocketTableModel(this);
...
void TestWindow::InitializeControls(void)
{
	...
	if(!m_tblSprockets)
	{
		m_tblSprockets = new GtTableView(this);
		rect.xMin = 200;
		rect.xMax = 600;
		rect.yMin = 50;
		rect.yMax = 500;
		if(m_tblSprockets)
		{
			m_tblSprockets->Set_objFrame(rect);
			m_tblSprockets->Set_objBackBrush(GtBrush(GtColor(190,90,160)));
			strCmd = "ListView";
			m_tblSprockets->Set_strText(strCmd);
			
			m_tblSprockets->SetTableModel(m_ptrSprocketModel);
			this->AddSubWidget(m_tblSprockets);	
		}
	}
...
void TestWindow::InitializeActions(void)
{
	...
	if(m_tblSprockets)
	{
		m_tblSprockets->SelectionChanged.SlotConnect(this,&TestWindow::OnLoadSprocket);
	}
};
...
void TestWindow::PopulateSprockets(void)
{
	...
	{
		for(j = 0; j < numSproc; j++)
		{
			Htl2DIndex index;
			sprintf(buf,"SPKT R%03dC%03d",i,j);
			strName = buf;
			...
			index.m_lngX = i; index.m_lngY = j;
			m_arrList01.insert(index,ptrNew);
		}
	}
	...
	for(i = 0; i < numSproc; i++)
	{
		for(j = 0; j < numSproc; j++)
		{
			...
			index.m_lngX = i; index.m_lngY = j;
			m_arrList02.insert(index,ptrNew);
		}
	}
	...
	for(i = 0; i < numSproc; i++)
	{
		for(j = 0; j < numSproc; j++)
		{
			...
			index.m_lngX = i; index.m_lngY = j;
			m_arrList03.insert(index,ptrNew);
		}
	}
...
void TestWindow::OnLoadSprocket(void)
{
	...
	GtTableModelIndex* ptrSel = m_tblSprockets->GetSelection()->Get_ptrTopLeft();
	if(!ptrSel){return;};
