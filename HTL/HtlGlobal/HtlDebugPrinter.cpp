...
namespace HTL
{
	...
	int HtlDebugPrinter::DebugPrint(int intLevel, char *format, ...)
	{
		...
		try{
			...
			intReturn &= DebugPrintToTextEdit(format,args);
			va_end(args);
			return intReturn;
		}catch(...){
			return -1;
	...
	int HtlDebugPrinter::DebugPrintToStream(char *format, va_list args)
	{
		int intReturn = 0;
		try{
			//va_start( args, format );
	...
	int HtlDebugPrinter::DebugPrintToTextEdit(char *fmt, va_list args)
	{
		if(m_blnPrintToTextEdit)
		{
			return 0;
		};
		return 0;
