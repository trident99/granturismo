...
namespace GT
{
	namespace GtChart
	{
		...
		GtPlotCurve::GtPlotCurve(GtObject* ptrParent)
			:GtPlotItem(ptrParent)
		{

		};
			//!Virtual destructor
		GtPlotCurve::~GtPlotCurve(void)
		{

		};
	
		GtPlotCurve & GtPlotCurve::operator = (GtPlotCurve & rhs)
		{
			if(this == &rhs)
				return *this;
			...
			m_strType = "GtPlotCurve";
			//GtPlotCurve Initialization////////////////////
			size_t i, intNumPts;
		...
		bool GtPlotCurve::operator == (GtPlotCurve & rhs)
		{
		...
		void GtPlotCurve::Clear(void)
		{
			m_arrObjPoints.clear();
		};
		size_t GtPlotCurve::Size(void)
		{
			return m_arrObjPoints.size();
		};

		GtPoint3DF & GtPlotCurve::At(size_t intIndex)
		{
			size_t intNumPoints;
		...
		void GtPlotCurve::PushBack(GtPoint3DF & objNewPoint)
		{
			m_arrObjPoints.push_back(objNewPoint);
			return;
		};
		void GtPlotCurve::Round(int intNumDecimals)
		{
		...
		int GtPlotCurve::Draw( GtPainter *painter)
		{
			...
			if(numPts > 0)
			{
				...
				for(j = 0; j < (numPts - 1);j++)
				{
					...
					modX1 = (float)this->m_arrObjPoints.at(j+1).x;
					modY1 = (float)this->m_arrObjPoints.at(j+1).y;

					//point 0
					ptrHoriz->ModelToViewPt(modX0,vwX0);
					ptrVert->ModelToViewPt(modY0,vwY0);
					//point 1
					ptrHoriz->ModelToViewPt(modX1,vwX1);
					ptrVert->ModelToViewPt(modY1,vwY1);
					//draw the line segment
					painter->GtDrawLine((int)vwX0,(int)vwY0,(int)vwX1,(int)vwY1);
		...
		GtRectI & GtPlotCurve::BoundingModelRect(void)
		{
		...
		//!Get the bounding rectangle of the item
		GtRectI & GtPlotCurve::BoundingCanvasRect(void)
		{
